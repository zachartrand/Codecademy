...
# Changed the variable below from Codecademy
# which just had a variable to change manually.
num_people_in_room = int(input("How many people are in the room?\n"))
...
def simulate(num_people):
    ...
    for i in range(0, num_people):
        ...
        birthday = " ".join([month_choice, str(day)])
        # Store the birthday
    ...
    for i, birthday in enumerate(birthdays):
        if find_duplicates(birthdays, birthday, i):
            match = True
            break
    if not match:
...
def calculate_probability(num_people):
    ...
    if num_people < 2:
        ...
        print(f"\n\nThe probability that two people in a room of {num_people} people "
            + f"have the same birthday is nearly {rounded}%")

# Find the same birthday within our list of birthdays
def find_duplicates(birthdays_list, day, index):
    people = []
    for i, birthday in enumerate(birthdays_list):
        if birthday == day and i != index:
            people.append(i + 1)
    if people:
